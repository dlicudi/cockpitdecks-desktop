...
def _editor_command_roles(parameters: dict) -> list[dict]:
    roles: list[dict] = []
    for name, spec in _editor_normalize_parameters(parameters).items():
        if spec.get("role") != "command":
            continue
        role_id = name[len("command-") :] if name.startswith("command-") else name
        roles.append(
            {
                "id": role_id,
                "name": name,
                "label": spec.get("label") or _editor_label(name),
...
class ActivationBase(ABC):
    ...
    def editor_schema(cls) -> dict:
        ...
        return {
            ...
            "command_roles": _editor_command_roles(parameters),
